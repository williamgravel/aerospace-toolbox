function [ephem,iono,time,leap,file] = readNavigationOld(filename,opts)
...
cline = fgetl(fid);
if cline(7) == '.' % float -> version = X.XX
    finfo = textscan(cline(1:60),'%9.2f %*11c %1c %*19c %1c %*19c','Whitespace','');
else % integer -> version = X
    finfo = textscan(cline(1:60),'%6f %*14c %1c %*19c %1c %*19c','Whitespace','');
end
